...
train_transforms = transforms.Compose([
    ...
    transforms.Resize((380, 380)),  # EfficientNet-B4 Standard-Auflösung
    transforms.ToTensor()
])
...
model_path = "./efficientnet-b4-isic"
if os.path.exists(model_path):
    ...
    # Schritt 8: Modell laden (EfficientNet-B4) - HAUPTÄNDERUNG
...
for epoch in range(EPOCHS):
    ...
    if avg_val_loss < best_val_loss:
        ...
        early_stopping_counter = 0
        model.save_pretrained("./efficientnet-b4-isic")
        processor.save_pretrained("./efficientnet-b4-isic")

        # Modellordner zippen
        import shutil
        shutil.make_archive("./efficientnet-b4-isic", 'zip', "./efficientnet-b4-isic")
        print("Modell als ZIP-Datei gespeichert: efficientnet-b4-isic")
...
try:
    ...
    y_true_bin = np.zeros((len(y_true), len(present_classes)))
    for i, label in enumerate(y_true):
        class_idx = present_classes.index(label)
...
try:
    ...
    plt.tight_layout()
    plt.savefig('test_roc_curves_efficientnet_b4.png', dpi=300, bbox_inches='tight')
    plt.show()

    print(f"\nTest Macro AUC: {auc_test_macro:.4f} | Samples: {len(y_true_test)} | Klassen: {len(present_classes)}")
    print(f"ROC Kurven gespeichert als: test_roc_curves_efficientnet_b4.png")
